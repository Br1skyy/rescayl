...
import { fileAssetUrl } from "@/lib/file-asset";
...
const MainContent = ({
  ...
  const sanitizedUpscaledImagePath = useMemo(
    () => fileAssetUrl(upscaledImagePath),
    [upscaledImagePath],
  );
  ...
  const handleDrop = (e) => {
    ...
    const droppedPaths = window.electron.getDroppedPaths();
    const filePath = droppedPaths[0] ?? e.dataTransfer.files[0].path;
    ...
    if (
      ...
      if (!FEATURE_FLAGS.APP_STORE_BUILD) {
        if (!rememberOutputFolder) {
          setOutputPath((prev) => prev || dirname);
        }
      }
        ...
        if (
          ...
          reader.onload = async (event) => {
            ...
            if (result instanceof ArrayBuffer) {
              file.encodedBuffer = arrayBufferToBase64(result);
            } else {
