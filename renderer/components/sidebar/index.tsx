...
const Sidebar = ({
  ...
  const preserveFilename = useAtomValue(preserveFilenameAtom);
  const bgRemovalMode = useAtomValue(bgRemovalModeAtom);
  const bgRemovalModel = useAtomValue(bgRemovalModelAtom);
  ...
  const upscaylHandler = async () => {
    ...
    if (imagePath !== "" || batchFolderPath !== "") {
      ...
      if (batchMode) {
        ...
        // Double Upscayl (single image only)
  ...
  return (
    <>
      ...
      >
        <button
          className="absolute -right-0 top-1/2 z-50 -translate-y-1/2 translate-x-1/2 rounded-full bg-base-100 p-4 shadow-lg shadow-black/20"
